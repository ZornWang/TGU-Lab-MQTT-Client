package iot_sensor.Sensor;

import iot_sensor.util.Checkout;
...
public class TempSensor {
    ...
    public void upload(float value) {
        ...
        for (int i = 0; i < 4; i++) { //节点LED和KEY的状态值
            payload[11 + i] = 0x00;
        }
        payload[15] = 0x01; //传感器类型，0x0101
        payload[16] = 0x01;
        System.arraycopy(floatToBytes(value, true), 0, payload, 17, 4);//温度数值
        for (int i = 0; i < 4; i++) { //湿度数值
            payload[21 + i] = 0x00;
        }
     ...
     * @return
     */
    private static byte[] floatToBytes(float f, boolean lsb) {
    ...
    private static String getHexString(byte[] bt, boolean lsb) {
